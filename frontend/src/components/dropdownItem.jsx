...
function DropdownItem({title, routings}) {
    ...
    return (
        < div className='' onMouseLeave={()=>setToggle(false)}>
        <div className="relative inline-block text-left">
        <div>
            <button onMouseEnter={()=>setToggled(true)} onClick={()=>setToggled(!toggled)}  type="button" className="inline-flex whitespace-nowrap justify-center gap-x-1.5 lg:text-lg my-4 text-2xl rounded-md bg-transparent px-3 py-2 font-semibold hover:bg-slate-950 text-gray-400 shadow-sm  hover:bg-gray-50" id="menu-button" aria-expanded="true" aria-haspopup="true">
            {title}
...
{/*
   ...
   */}
  <div  onMouseLeave={()=>setToggled(false)} className={` ${toggled ? "transform opacity-100 scale-100" : "transform opacity-0 scale-95"} absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none `} role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabIndex="-1">
    <div className="py-1" role="none">
      {/* <!-- Active: "bg-gray-100 text-gray-900", Not Active: "text-gray-700" --> */}
      {
        Object.keys(routings).map((item, index) => {
            return <Link className="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabIndex="-1" id={`menu-item-${index}`} key="item" to={routings[item]}>{item}</Link>
        })
      }
